// ==========================================================================
// NOVA LUXURY FASHION - PARTY WEAR COLLECTION CONTROLLER
// ==========================================================================
...
function initStaticCardEvents() {
    ...
    container.addEventListener('click', (e) => {
        ...
        const product = (typeof window.getProductById === 'function' ? window.getProductById(id) : null) || {
            id: id,
            name: card.querySelector('.card-content p')?.innerText.trim() || 'Party Apparel',
            price: card.getAttribute('data-price') || 7999,
...
function initPartySubFilters() {
    ...
    filterButtons.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            applyPartyFilter(subfilter);
        });
    });
}

function applyPartyFilter(subfilter) {
    const container = document.querySelector('.item-shop');
    if (!container) return;

    let items = typeof window.getProductsByCategory === 'function' ?
        window.getProductsByCategory('party') : (window.NOVA_PRODUCTS || []);
    ...
    if (subfilter && subfilter !== 'all') {
        items = items.filter(item => {
            ...
            if (subfilter === 'velvet') {
                return sub === 'velvet' || name.includes('velvet') || name.includes('jacquard');
            } else if (subfilter === 'silk') {
                return sub === 'silk' || name.includes('silk') || name.includes('baroque');
            } else if (subfilter === 'tuxedo') {
                return sub === 'tuxedo' || name.includes('tuxedo') || name.includes('pant');
            } else if (subfilter === 'shoes') {
                return sub === 'shoes' || name.includes('shoe') || name.includes('loafer') || name.includes('bowtie');
            }
            return true;
        });
    }
    ...
    if (countEl) {
        countEl.innerHTML = `Showing <b>${items.length} Party Wear Styles</b>`;
    }
}
        ...
        btn.addEventListener('click', () => {
            ...
            if (email && email.includes('@')) {
                ...
                if (typeof showToast === 'function') {
                    showToast('Welcome to the Gala VIP List.');
                }
                setTimeout(() => {
