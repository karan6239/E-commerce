// ==========================================================================
// NOVA LUXURY FASHION - UTILITY JACKETS COLLECTION CONTROLLER
// ==========================================================================

document.addEventListener('DOMContentLoaded', () => {
    initUtilitySubFilters();
...
function initStaticCardEvents() {
    ...
    container.addEventListener('click', (e) => {
        ...
        const product = (typeof window.getProductById === 'function' ? window.getProductById(id) : null) || {
            id: id,
            name: card.querySelector('.card-content p')?.innerText.trim() || 'Utility Jacket',
            price: card.getAttribute('data-price') || 4299,
...
function initUtilitySubFilters() {
    ...
    filterButtons.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            applyUtilityFilter(subfilter);
        });
    });
}

function applyUtilityFilter(subfilter) {
    const container = document.querySelector('.item-shop');
    if (!container) return;

    let items = typeof window.getProductsByCategory === 'function' ?
        window.getProductsByCategory('utility') : (window.NOVA_PRODUCTS || []);
    ...
    if (subfilter && subfilter !== 'all') {
        items = items.filter(item => {
            ...
            if (subfilter === 'chore') {
                return sub === 'chore' || name.includes('chore') || name.includes('shacket') || name.includes('carpenter');
            } else if (subfilter === 'vests') {
                return sub === 'vests' || name.includes('vest');
            } else if (subfilter === 'field') {
                return sub === 'field' || name.includes('field') || name.includes('windbreaker') || name.includes('safari');
            } else if (subfilter === 'waxed') {
                return sub === 'waxed' || name.includes('waxed') || name.includes('heritage');
            }
            return true;
        });
    }
    ...
    if (countEl) {
        countEl.innerHTML = `Showing <b>${items.length} Utility Jacket Styles</b>`;
    }
}
        ...
        btn.addEventListener('click', () => {
            ...
            if (email && email.includes('@')) {
                ...
                if (typeof showToast === 'function') {
                    showToast('Thank you for joining the Utility Gear Guild.');
                }
                setTimeout(() => {
