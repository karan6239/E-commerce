// ==========================================================================
// NOVA LUXURY FASHION - BEACH & COASTAL RESORT COLLECTION CONTROLLER
// ==========================================================================
...
function initStaticCardEvents() {
    ...
    container.addEventListener('click', (e) => {
        ...
        const product = (typeof window.getProductById === 'function' ? window.getProductById(id) : null) || {
            id: id,
            name: card.querySelector('.card-content p')?.innerText.trim() || 'Beach Apparel',
            price: card.getAttribute('data-price') || 1799,
...
function initBeachSubFilters() {
    ...
    filterButtons.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            applyBeachFilter(subfilter);
        });
    });
}

function applyBeachFilter(subfilter) {
    const container = document.querySelector('.item-shop');
    if (!container) return;

    let items = typeof window.getProductsByCategory === 'function' ?
        window.getProductsByCategory('beach') : (window.NOVA_PRODUCTS || []);
    ...
    if (subfilter && subfilter !== 'all') {
        items = items.filter(item => {
            ...
            if (subfilter === 'swim') {
                return sub === 'swim' || name.includes('swim') || name.includes('boardshort') || name.includes('beach short');
            } else if (subfilter === 'shirts') {
                return sub === 'shirts' || name.includes('camp') || name.includes('floral') || name.includes('shirt');
            } else if (subfilter === 'crochet') {
                return sub === 'crochet' || name.includes('crochet') || name.includes('tee') || name.includes('surf');
            } else if (subfilter === 'accessories') {
                return sub === 'accessories' || name.includes('hat') || name.includes('slide') || name.includes('shoe');
            }
            return true;
        });
    }
    ...
    if (countEl) {
        countEl.innerHTML = `Showing <b>${items.length} Beach & Resort Styles</b>`;
    }
}
        ...
        btn.addEventListener('click', () => {
            ...
            if (email && email.includes('@')) {
                ...
                if (typeof showToast === 'function') {
                    showToast('Welcome to the Coastal Club.');
                }
                setTimeout(() => {
