// ==========================================================================
// NOVA LUXURY FASHION - STREETWEAR COLLECTION CONTROLLER
// ==========================================================================

document.addEventListener('DOMContentLoaded', () => {
    initStreetSubFilters();
...
function initStaticCardEvents() {
    ...
    container.addEventListener('click', (e) => {
        ...
        const product = (typeof window.getProductById === 'function' ? window.getProductById(id) : null) || {
            id: id,
            name: card.querySelector('.card-content p')?.innerText.trim() || 'Streetwear Apparel',
            price: card.getAttribute('data-price') || 2899,
...
function initStreetSubFilters() {
    ...
    filterButtons.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            applyStreetFilter(subfilter);
        });
    });
}

function applyStreetFilter(subfilter) {
    const container = document.querySelector('.item-shop');
    if (!container) return;

    let items = typeof window.getProductsByCategory === 'function' ?
        window.getProductsByCategory('streetwear') : (window.NOVA_PRODUCTS || []);
    ...
    if (subfilter && subfilter !== 'all') {
        items = items.filter(item => {
            ...
            if (subfilter === 'hoodies') {
                return sub === 'hoodies' || name.includes('hoodie') || name.includes('sweatshirt');
            } else if (subfilter === 'cargos') {
                return sub === 'cargos' || name.includes('cargo') || name.includes('skate') || name.includes('track');
            } else if (subfilter === 'tees') {
                return sub === 'tees' || name.includes('tee') || name.includes('t-shirt');
            } else if (subfilter === 'jackets') {
                return sub === 'jackets' || name.includes('bomber') || name.includes('jacket') || name.includes('shoe') || name.includes('high-top');
            }
            return true;
        });
    }
    ...
    if (countEl) {
        countEl.innerHTML = `Showing <b>${items.length} Streetwear Styles</b>`;
    }
}
        ...
        btn.addEventListener('click', () => {
            ...
            if (email && email.includes('@')) {
                ...
                if (typeof showToast === 'function') {
                    showToast('Thank you for joining the Streetwear Syndicate.');
                }
                setTimeout(() => {
