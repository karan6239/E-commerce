// ==========================================================================
// NOVA LUXURY FASHION - OLD MONEY COLLECTION CONTROLLER
// ==========================================================================

document.addEventListener('DOMContentLoaded', () => {
    initOldMoneySubFilters();
...
function initStaticCardEvents() {
    ...
    container.addEventListener('click', (e) => {
        ...
        const product = (typeof window.getProductById === 'function' ? window.getProductById(id) : null) || {
            id: id,
            name: card.querySelector('.card-content p')?.innerText.trim() || 'Old Money Apparel',
            price: card.getAttribute('data-price') || 3999,
...
function initOldMoneySubFilters() {
    ...
    filterButtons.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            applyOldMoneyFilter(subfilter);
        });
    });
}

function applyOldMoneyFilter(subfilter) {
    const container = document.querySelector('.item-shop');
    if (!container) return;

    let items = typeof window.getProductsByCategory === 'function' ?
        window.getProductsByCategory('oldmoney') : (window.NOVA_PRODUCTS || []);
    ...
    if (subfilter && subfilter !== 'all') {
        items = items.filter(item => {
            ...
            if (subfilter === 'knits') {
                return sub === 'knits' || name.includes('knit') || name.includes('sweater') || name.includes('quarter-zip');
            } else if (subfilter === 'pants') {
                return sub === 'pants' || name.includes('trouser') || name.includes('chino') || name.includes('pant');
            } else if (subfilter === 'polos') {
                return sub === 'polos' || name.includes('polo') || name.includes('loafer') || name.includes('shoe');
            } else if (subfilter === 'blazers') {
                return sub === 'blazers' || name.includes('blazer') || name.includes('jacket') || name.includes('tweed');
            }
            return true;
        });
    }
    ...
    if (countEl) {
        countEl.innerHTML = `Showing <b>${items.length} Old Money Styles</b>`;
    }
}
        ...
        btn.addEventListener('click', () => {
            ...
            if (email && email.includes('@')) {
                ...
                if (typeof showToast === 'function') {
                    showToast('Welcome to the Heritage Salon.');
                }
                setTimeout(() => {
