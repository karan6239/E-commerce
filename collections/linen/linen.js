// ==========================================================================
// NOVA LUXURY FASHION - LINEN COLLECTION CONTROLLER
// ==========================================================================
...
function initStaticCardEvents() {
    ...
    container.addEventListener('click', (e) => {
        ...
        const product = (typeof window.getProductById === 'function' ? window.getProductById(id) : null) || {
            id: id,
            name: card.querySelector('.card-content p')?.innerText.trim() || 'Linen Apparel',
...
function initLinenSubFilters() {
    ...
    filterButtons.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            applyLinenFilter(subfilter);
        });
    });
}

function applyLinenFilter(subfilter) {
    const container = document.querySelector('.item-shop');
    if (!container) return;

    let items = typeof window.getProductsByCategory === 'function' ?
        window.getProductsByCategory('linen') : (window.NOVA_PRODUCTS || []);
    ...
    if (subfilter && subfilter !== 'all') {
        items = items.filter(item => {
            ...
            if (subfilter === 'shirts') {
                return sub === 'shirts' || name.includes('shirt') || name.includes('tee');
            } else if (subfilter === 'pants') {
                return sub === 'pants' || name.includes('trouser') || name.includes('pant');
            } else if (subfilter === 'layers') {
                return sub === 'layers' || name.includes('jacket') || name.includes('chore') || name.includes('overshirt');
            } else if (subfilter === 'blazers') {
                return sub === 'blazers' || name.includes('blazer');
            }
            return true;
        });
    }
    ...
    if (countEl) {
        countEl.innerHTML = `Showing <b>${items.length} Linen Fabric Styles</b>`;
    }
}
        ...
        btn.addEventListener('click', () => {
            ...
            if (email && email.includes('@')) {
                ...
                if (typeof showToast === 'function') {
                    showToast('Thank you for joining the Linen Club.');
                }
                setTimeout(() => {
