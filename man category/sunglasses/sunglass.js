...
function initProductInteractions() {
    ...
    cards.forEach((card, index) => {
        ...
        const category = catEl?.innerText.trim().toLowerCase() || document.querySelector('.item-shop')?.getAttribute('data-category') || 'sunglasses';

        const productObj = {
            id: card.getAttribute('data-id') || (index + 400),
            name: name,
            price: priceText,
            priceNum: priceNum,
            image: image,
            category: category,
            description: `Exquisitely crafted ${name} featuring signature styling, UV400 protection, and exceptional durability.`
        };
        ...
        if (imgEl) {
            imgEl.onerror = () => {
                imgEl.src = 'https://i.pinimg.com/736x/6f/39/58/6f395823b7bc10092301881393806af5.jpg';
            };
        }
    });
}
